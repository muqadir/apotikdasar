...
class PembelianController extends Controller
{
    public function index () {
        ...
        if (Auth::user()->isAbleTo('pembelian-read')) {
    ...
    {
        if (Auth::user()->isAbleTo('pembelian-create')) {
            $rules = [
            ...
            if ($save) {
                $stock = Stockobat::where('obat_id', $request->item)->first();
    ...
    {
        if (Auth::user()->isAbleTo('pembelian-read|pembelian-delete')) {
            $id = $request->id;
        
            $data = Pembelian::find($id);
            $hapus = $data->delete();
            if ($hapus) {
                $stock = Stockobat::where('obat_id', $data->item)->first();
